...
2. Clona el repo nuevo y ejecuta el script de inicializaciÃ³n desde su raÃ­z:
   ...
   - reemplaza todos los tokens `interaction`, `Interaction`,
     `Interaction` y `Sistema de interacción de objetos para aventura gráfica: combinación de items mediante recetas (válidas e inválidas), sockets de uno o varios slots con aceptación por tipo, máquinas de estados discretos y contenedores revelables. C# puro, agnóstico al motor y al contenido del juego; los objetos se definen como datos, no como código.` en contenidos y nombres de
     archivo,
...
- El asmdef de `Runtime/` (`EnriRanjan.Interaction.asmdef`) tiene
...
como vista), cambia el flag en `Runtime/EnriRanjan.Interaction.asmdef`:
...
|---------------------|--------------------------------------------------|---------------|
| `interaction`    | `package.json`, nombres de archivo, docs         | `mypackage`   |
| `Interaction`  | asmdefs, namespaces C#, nombres de archivo       | `MyPackage`   |
| `Interaction`  | `package.json`, README/CHANGELOG generados       | `My Package`  |
| `Sistema de interacción de objetos para aventura gráfica: combinación de items mediante recetas (válidas e inválidas), sockets de uno o varios slots con aceptación por tipo, máquinas de estados discretos y contenedores revelables. C# puro, agnóstico al motor y al contenido del juego; los objetos se definen como datos, no como código.`   | `package.json`                                   | descripciÃ³n corta |
